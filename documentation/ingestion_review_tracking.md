...
| ID | Area | Issue | Suggested Fix | Priority | Status |
|---|---|---|---|---|---|
| IMG-001 | core/ingest_collection.py / IMAGES/image_parser.py | OCR is enabled through core special case. | Move enable_ocr to filetype config. | High | Done |
...
| IMG-003 | IMAGES/image_serializer.py | Image payloads do not emit shared normalized identity fields. | Add file-aware generated identifier, identifier_field=image_file, identifier_namespace=image_file, identifier_kind=generated, primary_name, description, link_keys, and related_link_keys while preserving OCR/image metadata. | High | Done |
...
| ID | Area | Issue | Suggested Fix | Priority | Status |
|---|---|---|---|---|---|
| PDF-001 | PDF/pdf_detector.py | PDF mode/doc_type detection uses hardcoded thresholds and scanned glossary/reference pages can be misclassified as procedural. | Move PDF detection thresholds and keywords to config/pdf_detection_hints.json; add reference keyword signals such as glossary, appendix, index, definition, introduction, overview. | High | Done |
...
| RET-013 | Final answer candidate selection mismatch | Semantic retrieval can find the correct note in top candidates, but final rerank/answer synthesis may select an unrelated note as the returned answer. Example: “how are options fetched in CRD” had “Fetching Options” in debug candidates but final answer used “Jennison SOD Checkout and Data Verification.” | Review reranking and final answer payload selection. Add debug visibility showing why final answer payload was selected, and prefer strong title/keyword overlap for procedural/note questions. | Critical | Not Started |
...
| UI-004 | Collection editor asset roots | Create/Edit Collection does not expose asset_search_roots, even though DOCS parser supports them for Obsidian image resolution. | Add an Asset search roots text field to collection config UI and save it to collections.json. | High | Done |
