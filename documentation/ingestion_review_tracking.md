...
| RET-013 | Final answer/debug path mismatch | Final rerank can show the correct note as top candidate, but the final answer may come from a different payload because answer retrieval and debug retrieval are run separately. | Unify answer and debug generation so final answer, selected payload, and debug candidates come from the same retrieval run. Add selected_answer_payload and selected_answer_reason to debug. | Critical | Not Started |
