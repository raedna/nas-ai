...
| RET-013 | Final answer/debug path mismatch | Debug rerank can show the correct note as top candidate, but the final answer may come from a different payload because the UI runs answer retrieval and debug retrieval separately. | Unify answer and debug generation so the final answer payload, final reranked top candidate, and returned answer are produced from the same retrieval run. Add debug fields for selected_answer_payload and selected_answer_reason. | Critical | Not Started |
