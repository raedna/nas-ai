...
| DOC-014 | Render related images in answers | Answers can list related image names/paths, but the UI does not yet display the actual screenshots/images. This affects embedded doc images and standalone image ingestions. | Pass image paths from returned payloads to the Ask UI and render them under the final answer using st.image(), with captions. For standalone image results, show the image plus OCR/caption/metadata. | High | Not Started |
