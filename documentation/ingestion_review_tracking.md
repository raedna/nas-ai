...
| DOC-014 | Render related images and OCR in answers | Answers can list related image names, but payloads do not yet consistently preserve resolved image paths, so the UI cannot render actual screenshots/images. | Ensure doc parser/serializer stores embedded_image_targets, embedded_image_paths, and OCR metadata in doc payloads; then render images in Ask UI with OCR in collapsed expanders. | High | Not Started |
