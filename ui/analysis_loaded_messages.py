...
def render_analysis_panel():
    ...
    def workspace_message_key(message):
        persistent_id = message.get("id") or message.get("message_id")
        if persistent_id is not None:
            return ("saved", persistent_id)

        return (
            message.get("source_type") or "workspace",
            message.get("session_id"),
            message.get("message_index"),
            message.get("msg_seq_num"),
    ...
    def add_messages_to_workspace(messages, source_label="message"):
        ...
        for message in messages or []:
            ...
            key = workspace_message_key(candidate)
            if key in existing_keys:
                continue
            rows_to_add.append(candidate)
        ...
        if rows_to_add:
            ui.notify(
                f"Added {len(rows_to_add)} {source_label}(s) to the investigation workspace.",
                color="positive",
            )
        else:
            ui.notify(
                "The selected message(s) are already in the workspace, or the workspace is full.",
                color="info",
            )
    ...
    def render_loaded_messages(messages, title="Loaded Messages", expanded=False):
        """Render live analysis messages with the same selection workflow as saved messages."""
        source_messages = [dict(message) for message in (messages or [])]
        if not source_messages:
            return

        rows = []
        for fallback_index, message in enumerate(source_messages, start=1):
            message_index = message.get("message_index") or fallback_index
            rows.append({
                **message,
                "message_index": message_index,
                "route": message.get("route") or f"{message.get('sender') or ''} → {message.get('target') or ''}",
                "time": message.get("transact_time") or message.get("sending_time"),
            })
        ...
        with ui.expansion(
            ...
            def view_selected_message_tags():
                ...
                if len(selected) != 1:
                    ui.notify("Select one loaded message first.", color="warning")
                    return

                raw_text = selected[0].get("raw_text") or ""
                if not raw_text:
                    ui.notify("The selected loaded message has no raw FIX text.", color="negative")
                    return

                decoded = analyze_fix_message(raw_text)
                tag_rows = decoded.get("decoded_rows") or []

                tag_result_area.clear()
                with tag_result_area:
                    ui.label(
                        f"Decoded Tags for Loaded Message {selected[0].get('message_index')}"
            ...
            def compare_selected_messages():
                selected = selected_rows()
                if len(selected) != 2:
                    ui.notify("Select exactly two loaded messages to compare.", color="warning")
                    return

                raw_first = selected[0].get("raw_text") or ""
                raw_second = selected[1].get("raw_text") or ""
            ...
            def add_selected_to_workspace():
                selected = selected_rows()
                if not selected:
                    ui.notify("Select at least one loaded message first.", color="warning")
                    return
                add_messages_to_workspace(selected, source_label="loaded message")
            ...
            with ui.row().classes("q-gutter-sm q-pa-sm"):
                ...
                ui.button(
                    "Compare Selected Loaded Messages",
                ...
                with saved_session_viewer:
                    ...
                    def add_selected_saved_message_to_compare():
                        ...
                        if not selected_rows:
                            ui.notify("Select at least one saved message first.", color="warning")
                            return

                        add_messages_to_workspace(selected_rows, source_label="saved message")
    ...
    with control_area:
        ...
        def add_current_analysis_to_workspace():
            if not live_analysis_messages:
                ui.notify("Run an analysis first, then add its message(s) to the workspace.", color="warning")
                return
            add_messages_to_workspace(live_analysis_messages, source_label="current message")

        def compare_workspace_pair():
            if len(workspace_messages) != 2:
        ...
        def compare_workspace_multi():
            if len(workspace_messages) < 3:
            ...
            with reporting_area:
                ui.label("Summary").classes("text-lg font-semibold")
                ui.markdown(_format_summary_with_pipes(sequence_result.get("summary"))).classes(
                    "w-full q-pa-md bg-grey-1 rounded-borders"
                )
                render_sequence_insights(insights)
            ...
            ui.notify(
                f"Analyzed {len(raw_messages)} workspace messages as a multi-message sequence.",
                color="positive",
            )
        ...
        ui.separator().classes("q-my-sm")
        ui.label("Control").classes("text-sm font-semibold")

        ui.button(
            "Add Current to Workspace",
            on_click=add_current_analysis_to_workspace,
        ).props("outline color=secondary").classes("w-full")

        ui.button(
            "Compare 2 Workspace Messages",
            on_click=compare_workspace_pair,
        ).props("outline color=primary").classes("w-full")

        ui.button(
            "Multi-Message Compare",
            on_click=compare_workspace_multi,
        ).props("outline color=primary").classes("w-full")
