...
def entry_page(chunk_id: str):
    ...
    # Structured records are single-row — render just this chunk. Documents
    # (notes/articles) merge all sibling chunks: same identifier when present
    # (kb articles), else same source_file (obsidian notes).
    if (r["doc_type"] or p.get("doc_type")) == "structured":
        doc_rows = rows
    elif r["identifier"]:
        doc_rows = fetchall(
            "SELECT payload FROM chunks WHERE collection_name = %s AND identifier = %s ORDER BY id",
            (r["collection_name"], r["identifier"]))
    else:
        doc_rows = fetchall(
            "SELECT payload FROM chunks WHERE collection_name = %s AND source_file = %s ORDER BY id",
